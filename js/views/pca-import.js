...
function openPersonnelImportModal() {
  ...
  document.getElementById('import-modal-body').innerHTML = `
    <div class="import-info">
      ...
      <table class="import-cols-table">
        ...
        <tbody>
          <tr><td>nome</td><td>✓</td><td>Testo</td><td>—</td></tr>
          <tr><td>cognome</td><td>✓</td><td>Testo</td><td>—</td></tr>
          <tr><td>cf</td><td></td><td>Testo</td><td>Codice fiscale. Usato per rilevare duplicati</td></tr>
          <tr><td>comitato</td><td></td><td>Testo</td><td>—</td></tr>
          <tr><td>numero</td><td></td><td>Numero</td><td>Solo cifre</td></tr>
          <tr><td>email</td><td></td><td>Email</td><td>—</td></tr>
          <tr><td>qualifiche</td><td></td><td>Testo</td><td>—</td></tr>
          <tr><td>ruolo</td><td></td><td>Testo</td><td>Autista · Soccorritore · Infermiere · Medico · OPEM · Coordinatore · Altro</td></tr>
          <tr><td>risorsa</td><td></td><td>Testo</td><td>Nome risorsa (es. ASM-01). Viene convertito in UUID</td></tr>
          <tr><td>ice</td><td></td><td>Testo</td><td>Contatto emergenza</td></tr>
          <tr><td>allergie</td><td></td><td>Testo</td><td>—</td></tr>
          <tr><td>attivazione</td><td></td><td>Testo</td><td>Protocolli di attivazione</td></tr>
        </tbody>
      </table>
      <p class="import-info-desc" style="margin-top:10px;">
        Se un personale con lo stesso <strong>cf</strong> esiste già, verrà aggiornato.
        Se cf è vuoto, verrà creato un nuovo record.
      </p>
    </div>
    <div class="import-drop-zone" id="personnel-drop-zone">
      <div class="import-drop-icon">📄</div>
      <div class="import-drop-text">Trascina il CSV qui o clicca per selezionare</div>
      <input type="file" id="personnel-csv-input" accept=".csv" style="display:none" />
    </div>
    <div id="personnel-import-preview" style="display:none;">
      <div class="import-preview-header">
        <span id="personnel-preview-count"></span>
        <button class="btn-secondary" onclick="resetPersonnelImport()">✕ Cambia file</button>
      </div>
      <div class="import-preview-scroll">
        <table class="import-preview-table" id="personnel-preview-table"></table>
      </div>
    </div>
    <div id="personnel-import-error" class="import-error"></div>
    <div id="personnel-import-progress" style="display:none;">
      <div class="import-progress-bar">
        <div class="import-progress-fill" id="personnel-progress-fill"></div>
      </div>
      <div class="import-progress-text" id="personnel-progress-text">Importazione...</div>
    </div>`;

  // Wire drop zone
  const dropZone = document.getElementById('personnel-drop-zone');
  const fileInput = document.getElementById('personnel-csv-input');
  ...
  dropZone.addEventListener('drop', e => {
    ...
    if (file) handlePersonnelCSV(file);
  });
  fileInput.addEventListener('change', e => {
    if (e.target.files[0]) handlePersonnelCSV(e.target.files[0]);
  });

  // Wire confirm button
  document.getElementById('import-modal-confirm').style.display = 'none';
  document.getElementById('import-modal-confirm').onclick = confirmPersonnelImport;

  document.getElementById('modal-import').classList.remove('hidden');
}

let _personnelImportRows = [];

async function handlePersonnelCSV(file) {
  const text = await file.text();
  const errEl = document.getElementById('personnel-import-error');
  errEl.textContent = '';
  ...
  if (!headers.includes('nome') || !headers.includes('cognome')) {
    errEl.textContent = 'Il CSV deve contenere le colonne "nome" e "cognome".';
    return;
  }

  // Fetch resources for name→id resolution
  const { data: resources } = await db
    .from('resources')
    .select('id, resource')
    .eq('event_id', PCA.eventId);
  const resourceMap = {};
  (resources || []).forEach(r => { resourceMap[r.resource.toLowerCase()] = r.id; });
  ...
  rows.forEach((row, i) => {
    ...
    const cognome = cell(row, headers, 'cognome')?.trim();
    if (!nome || !cognome) {
      warnings.push(`Riga ${lineNum}: nome e cognome obbligatori — saltata`);
      return;
    }

    const risorsaName = cell(row, headers, 'risorsa')?.trim();
    let risorsaId = null;
    if (risorsaName) {
      risorsaId = resourceMap[risorsaName.toLowerCase()];
      if (!risorsaId) warnings.push(`Riga ${lineNum}: risorsa "${risorsaName}" non trovata — campo ignorato`);
    }

    parsed.push({
      nome, cognome,
      cf:             cell(row, headers, 'cf')?.trim().toUpperCase() || null,
      comitato:       cell(row, headers, 'comitato')?.trim() || null,
      numero:         cell(row, headers, 'numero')?.trim() || null,
      email:          cell(row, headers, 'email')?.trim() || null,
      qualifiche:     cell(row, headers, 'qualifiche')?.trim() || null,
      ruolo:          cell(row, headers, 'ruolo')?.trim() || null,
      risorsa_id:     risorsaId,
      ice:            cell(row, headers, 'ice')?.trim() || null,
      allergie:       cell(row, headers, 'allergie')?.trim() || null,
      attivazione:    cell(row, headers, 'attivazione')?.trim() || null,
    });
  });

  _personnelImportRows = parsed;
  ...
  // Show preview
  document.getElementById('personnel-drop-zone').style.display = 'none';
  document.getElementById('personnel-import-preview').style.display = 'block';
  document.getElementById('personnel-preview-count').textContent =
    `${parsed.length} record da importare`;
  ...
  table.innerHTML = `
    ...
    <tbody>
      ${parsed.map(p => `<tr>
        ...
        <td>${p.ruolo || '—'}</td>
        <td>${p.risorsa_id
          ? (resources || []).find(r => r.id === p.risorsa_id)?.resource || p.risorsa_id
          : '—'}</td>
        <td>${p.comitato || '—'}</td>
        <td>${p.email || '—'}</td>
      </tr>`).join('')}
...
function resetPersonnelImport() {
  _personnelImportRows = [];
  document.getElementById('personnel-drop-zone').style.display = 'flex';
  document.getElementById('personnel-import-preview').style.display = 'none';
  document.getElementById('personnel-import-error').textContent = '';
  document.getElementById('import-modal-confirm').style.display = 'none';
  document.getElementById('personnel-csv-input').value = '';
}

async function confirmPersonnelImport() {
  if (_personnelImportRows.length === 0) return;

  const confirmBtn = document.getElementById('import-modal-confirm');
  confirmBtn.disabled = true;
  const progressEl = document.getElementById('personnel-import-progress');
  const fillEl     = document.getElementById('personnel-progress-fill');
  const textEl     = document.getElementById('personnel-progress-text');
  progressEl.style.display = 'block';

  // Fetch existing personnel by CF for conflict detection
  const cfs = _personnelImportRows.map(p => p.cf).filter(Boolean);
  let existingByCF = {};
  if (cfs.length > 0) {
  ...
  let done = 0;
  let errors = 0;
  const total = _personnelImportRows.length;

  for (const p of _personnelImportRows) {
    const payload = {
      name:                 p.nome,
      surname:              p.cognome,
      cf:                   p.cf,
    ...
    let error;
    const existingId = p.cf ? existingByCF[p.cf] : null;

    if (existingId) {
      ({ error } = await db.from('personnel').update(payload).eq('id', existingId));
    } else {
    ...
    textEl.textContent = `${done}/${total} record importati${errors > 0 ? ` (${errors} errori)` : ''}`;
  }

  confirmBtn.disabled = false;
  if (errors === 0) {
    showToast(`${done} record importati ✓`, 'success');
